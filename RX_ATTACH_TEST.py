...
logfilename = sys.argv[0].replace('.py', '').replace('.PY', '')
...
df = pd.concat((pd.read_csv(f, header = 0) for f in cmw_csv_files))
timestr_f = time.strftime("%Y%m%d-%H%M%S")
df.to_csv("{1}\\CMW_VERDICT_{0}.csv".format(timestr_f, mydir_d))

# converting csv file to json
df_j = pd.read_csv (r'{0}\\CMW_VERDICT_*.csv'.format(mydir_d))
df.to_json (r'{0}\\ctf_j.json'.format(mydir_d))
...
df_fsw.to_csv("{1}\\fsw_VERDICT_{0}.csv".format(timestr_f, mydir_d))

# converting fsw verdict csv file to json

df_f = pd.read_csv (r'{0}\\fsw_VERDICT_*.csv'.format(mydir_d))
df.to_json (r'{0}\\ctf_fsw.json'.format(mydir_d))
# printing the csv contents in run log .py for all variants
logger.debug(df_j)
