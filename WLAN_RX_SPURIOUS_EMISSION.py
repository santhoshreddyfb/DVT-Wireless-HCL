...
mydir = os.path.join('{0}\\Wifi_Logs_folder\\'.format(os.getcwd().split('D')[0]), '{0}_{1}_{2}'.format(sys.argv[1], datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'),os.getcwd().split('D')[0]))
os.makedirs(mydir)
...
try:
    ...
    # read json file
    data = None
    f_json = os.path.curdir + os.sep + 'CONFIG_WLAN_RX.json'
        ...
        if DAU_PRESENT:
            ...
            if DAU_PRESENT:
                ...
                if 1:
                    ...
                    rx_per = True
                    if rx_per:
                        logger.debug('INFO:*** RX PER TEST')
                        logger.debug('INFO:*** Set WLAN PER Standard Non HT, 6 Mbps')
                        cmw.write('CONFigure:WLAN:SIGN:PER:FDEF NHT, BW20,BR12')
                        logger.debug('INFO:*** Set WLAN Data pattern to PN9')
                        cmw.write('CONFigure:WLAN:SIGN:PER:DPATtern PN9')
                        logger.debug('INFO:*** Set WLAN Number Of Packet to 256')
                        cmw.write('CONFigure:WLAN:SIGN:PER:PACKets 256')
                        logger.debug('INFO:*** Set WLAN Packet Size to 1024 bytes')
                        cmw.write('CONFigure:WLAN:SIGN:PER:PAYLoad:SIZE 1024')
                        logger.debug('INFO:*** Set WLAN Time Interval to 10')
                        cmw.write('CONFigure:WLAN:SIGN:PER:DINTerval 10')
                        logger.debug('INFO:*** Set WLAN Signaling AP Output Power to -65 dBm')
                        cmw.write('CONFigure:WLAN:SIGN:RFSettings:BOPower -65')
                        ...
                        for index_pg in range(1, 4):
                            ...
                            if 'ON' in pg_state:
                                pg_state[0] = 'OFF'
                                pg_state = cmw.write(
                                    'CONFigure:WLAN:SIGN:PGEN{0}:CONFig {1}'.format(index_pg, ','.join(pg_state)))

                        cmw.write('TRIGger:WLAN:SIGN:RX:MACFrame:BTYPe ABURsts')
                        cmw.write('TRIGger:WLAN:SIGN:RX:MACFrame:RATE ALL')
                        # cmw.write('TRIGger:WLAN:SIGN:RX:MACFrame:RREStriction OFF')

                        logger.debug('INFO:*** Trig RX PER')
                        cmw.write('ABORt:WLAN:SIGN:PER')
                        cmw.write('INIT:WLAN:SIGN:PER')

                        logger.debug('INFO:*** Wait for RX PER measurement to complete')
                        while True:
                            ...
                            per_state = cmw.ask('FETCh:WLAN:SIGN:PER:STATe:ALL?')
                            if per_state[0] == 'RDY' and per_state[1] == 'ADJ' and per_state[2] == 'INV':
                                logger.debug('INFO:*** RX PER measurement Ready')
                                break

                            logger.debug('INFO:*** Get PER results')
                        buffin = cmw.ask('FETCh:WLAN:SIGN:PER?')
